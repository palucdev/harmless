...
export const fsReadInputSchema = z
  .object({
    path: z
      .string()
      .min(1)
      .describe('Relative path to file or directory. Examples: "." (current dir), "docs/", "src/index.ts". ' + 'For directories: returns entries. For files: returns content with line numbers.'),
    ...
    limit: z.number().int().min(1).max(2000).optional().default(100).describe('Max entries to return for directory listings (default 100).'),

    offset: z.number().int().min(0).optional().default(0).describe('Skip the first N matching entries for directory listings (default 0).'),

    lines: z
      .string()
      .optional()
      .describe('Limit file reading to specific lines. Format: "10" (single line), "10-50" (range). ' + 'Useful for large files or when you know the target area.'),

    depth: z.number().int().min(1).max(20).optional().describe('Directory listing depth (default 1).'),

    details: z
      .boolean()
      .optional()
      .default(false)
      .describe('Include file details (size, modified time) in directory listings. ' + 'Default false for compact output.'),

    types: z.array(z.string()).optional().describe('Filter directory listing by file type. Examples: ["ts", "js"], ["md"].'),

    glob: z.string().optional().describe('Glob pattern filter for directory listings. Example: "**/*.ts".'),
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (stat.isDirectory()) {
          ...
          const includeDir = (!options.glob || matchesGlob(itemRelPath, options.glob)) && options.includeDirs;
          if (includeDir) {
            const entry: TreeEntry = {
              path: itemRelPath,
              kind: 'directory',
...
async function readFile(absPath: string, relativePath: string, options: { lines?: string }): Promise<FsReadResult> {
      ...
      const result: FsReadResult = {
        ...
        hint: isAbsolute
          ? `This is a SANDBOXED filesystem — absolute paths are not allowed. ` +
            (suggestedPath ? `Try: fs_read("${suggestedPath}") — use relative paths without leading "/". ` : '') +
        ...
        if (autoResolve.resolved && autoResolve.resolvedPath) {
          ...
          if (autoMode !== 'content') {
            const result: FsReadResult = {
              ...
              hint: 'This path points to a file. Use mode="content" (or omit mode) to read it, ' + 'or use fs_read on a directory path for listing.',
            };
            return result;
          }
