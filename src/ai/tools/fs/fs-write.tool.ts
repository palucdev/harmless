...
export const fsWriteInputSchema = z
  .object({
    path: z
      .string()
      .min(1)
      .describe('Relative path to the file. For create: where to create. For update: file to modify. ' + 'Parent directories are created automatically for new files.'),

    operation: z
      .enum(['create', 'update'])
      .describe('REQUIRED. The operation type: ' + '"create" = make new file (fails if exists), ' + '"update" = modify existing file (requires "action" and "lines" parameters).'),

    // Targeting (for update)
    lines: z
      .string()
      .optional()
      .describe('REQUIRED for update. Target specific lines. Format: "10" (line 10), "10-15" (lines 10-15 inclusive). ' + 'Get line numbers from fs_read output.'),
    ...
    content: z
      .string()
      .optional()
      .describe('The content to write. Required for create, replace, insert_before, insert_after. ' + 'Not needed for delete_lines.'),

    // Safety
    checksum: z
      .string()
      .optional()
      .describe('Expected checksum of the current file (from previous fs_read). ' + 'If provided and file has changed, operation fails. STRONGLY RECOMMENDED for updates.'),

    dryRun: z
      .boolean()
      .optional()
      .default(false)
      .describe('If true, returns what WOULD change without applying it. ' + 'Returns a unified diff. Use to preview and verify edits.'),

    createDirs: z.boolean().optional().default(true).describe('For create: whether to create parent directories if missing. Default true.'),
  })
  ...
  .refine(
    ...
    {
      message: '"action" parameter is required when operation="update". Use action="replace", "insert_before", "insert_after", or "delete_lines".',
      path: ['action'],
    }
  )
  .refine(
...
async function createFile(absPath: string, relativePath: string, content: string, options: { createDirs: boolean; dryRun: boolean }): Promise<FsWriteResult> {
  // Check if exists
  if (await fileExists(absPath)) {
    return {
      ...
      error: {
        ...
        recoveryHint: 'To modify this file, first call fs_read to get its content and checksum, then use fs_write with operation="update".',
      },
...
async function updateFile(absPath: string, relativePath: string, input: FsWriteInput): Promise<FsWriteResult> {
  // Check exists
  if (!(await fileExists(absPath))) {
    return {
      status: 'error',
      ...
      error: {
        ...
        recoveryHint: 'Use fs_write with operation="create" and provide the content to create a new file.',
      },
  ...
  if (!input.lines) {
    return {
      ...
      error: {
        ...
        recoveryHint: 'Add lines="N" or lines="N-M" to specify which lines to target. Get line numbers from fs_read output.',
      },
    ...
    default:
      return {
        ...
        error: {
          ...
          recoveryHint: 'Use one of: action="replace", action="insert_before", action="insert_after", action="delete_lines".',
        },
  ...
  handler: async (args: unknown): Promise<unknown> => {
    ...
    if (!parsed.success) {
      const result: FsWriteResult = {
        ...
        operation: (args as { operation?: 'create' | 'update' })?.operation ?? 'create',
        error: {
          code: 'INVALID_INPUT',
          message: parsed.error.issues.map((e) => `${e.path.join('.')}: ${e.message}`).join(', '),
          recoveryHint: 'Check the input parameters and try again.',
        },
