...
const ALWAYS_EXCLUDE = ['.git', 'node_modules', '.svelte-kit', '.next', '.nuxt', '__pycache__', 'target', 'dist', '.agent-data'];
...
class FileIndexManager {
  private indexes: Map<string, FileIndex> = new Map<string, FileIndex>();
 ...
 */
export async function searchFiles(root: string, query: string, options: FileSearchOptions = {}): Promise<FileSearchResult[]> {
 ...
 */
export async function tryAutoResolve(root: string, relativePath: string, options: FileSearchOptions = {}): Promise<AutoResolveResult> {
...
export async function rebuildFileIndex(root: string, options: FileSearchOptions = {}): Promise<FileIndex> {
