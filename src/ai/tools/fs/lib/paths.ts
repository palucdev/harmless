...
import { config, type Mount } from '../env';
...
export type PathResolutionResult = { ok: true; resolved: ResolvedPath } | { ok: false; error: string };
...
export function resolvePath(virtualPath: string): PathResolutionResult {
  ...
  if (isAbsolutePath(trimmed)) {
    ...
    const suggestion = suggestedPath ? `Try using "${suggestedPath}" instead (without the leading "/").` : `Use fs_read(".") to explore available mounts.`;

    return {
      ok: false,
      error: `SANDBOXED FILESYSTEM: Absolute paths like "${trimmed}" are not allowed. ` + `This tool only accesses specific mounted directories. ` + `Available mounts: ${mounts}. ${suggestion}`,
    };
  }
  ...
  if (!mount) {
    ...
    if (config.MOUNTS.length === 1) {
      ...
      if (!absolutePath.startsWith(singleMount.absolutePath + path.sep) && absolutePath !== singleMount.absolutePath) {
    ...
    return {
      ok: false,
      error: `Path "${normalized}" does not match any mount. Available mounts: ${availableMounts}. ` + `Paths must start with a mount name (e.g., "${config.MOUNTS[0]?.name}/file.md").`,
    };
  }
  ...
  // Security check: ensure path is within mount
  if (!absolutePath.startsWith(mount.absolutePath + path.sep) && absolutePath !== mount.absolutePath) {
    return { ok: false, error: 'Path is outside allowed directory' };
  }
 ...
 */
export async function validateSymlinks(absPath: string, mount: Mount): Promise<{ ok: true; realPath: string } | { ok: false; error: string }> {
  try {
    ...
    if (stat.isSymbolicLink()) {
      ...
      if (!resolvedTarget.startsWith(mount.absolutePath + path.sep) && resolvedTarget !== mount.absolutePath) {
        return {
          ok: false,
 ...
 */
export async function validatePathChain(absPath: string, mount: Mount): Promise<{ ok: true } | { ok: false; error: string }> {
